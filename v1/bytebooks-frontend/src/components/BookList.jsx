...
function BookList() {
  ...
  // ── CRUD view mode ──────────────────────────────────────────────────────
  // mode controls which view is shown: 'list' (default), 'add', or 'edit'
  const [mode, setMode] = useState('list');
  // The book currently being edited (only used when mode is 'edit')
  ...
  const handleDelete = async (book, e) => {
    ...
    try {
      ...
      if (response.status === 204) {
        ...
        alert(data.detail || `Failed to delete book (status ${response.status})`);
      }
    } catch (err) {
  ...
  if (mode === 'add') {
    return (
      <AddBookForm
        onBookAdded={() => { fetchBooks(); setMode('list'); }}
  ...
  if (mode === 'edit') {
    return (
      <EditBookForm
        key={editingBook.id}
        book={editingBook}
        onBookUpdated={() => { fetchBooks(); setMode('list'); }}
        onCancel={() => setMode('list')}
      />
    );
  }
  ...
  return (
    <div className="book-list-container">
      {/* ── Add New Book Button ──────────────────────────────────────── */}
      <button className="add-book-button" onClick={() => setMode('add')}>
        Add New Book
      </button>
